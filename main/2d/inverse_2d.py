...
def contourPlot(xtest_mesh, ytest_mesh, u_test, u_pred, N_test, i):
    ...
    plt.savefig(f'{dirpath}/inverse_2d_contour_iter{i+1}_new.pdf')
    plt.close(fig)
    with open(f'{dirpath}/inverse_2d_contour_upred_iter{i+1}.json', 'w') as f:
...
if __name__ == "__main__":
    # lambda_1 * u_xx + u_yy = 0, x in [0, 1], y in [0, 1] -> 2 * u_xx + u_yy = 0
    # u(0, y) = -2 * y^2            ## left
    # u(1, y) =  1 - 2 * y^2 + 3y   ## right
    # u(x, 0) = x^2                 ## bottom
    # u(x, 1) = x^2 - 2 + 3x        ## top
    # analytical solution: u(x, y) = x^2 - 2(lambda_1) * y^2 + 3xy
    ...
    # settings for plots 
    dirpath = f'./main/2d/inverse_2d_figures/{start_time}' # where figures are stored
    os.mkdir(dirpath)
    ticksize = 8.5
    ...
    # Plot 1. Boundary Point, Collocation Point, Observed Data
    ...
    od, = plt.plot(xo_grid, yo_grid, 'D', c='#ab95e6', label = 'Observed Data', clip_on=False)
    plt.gca().set(xlim=(0, 1), ylim=(0, 1), xlabel='x', ylabel='y', title='Training Data')
    plt.figlegend(handles=[bc, cp, od], loc='center right', bbox_to_anchor=(0.5, 0., 0.5, 0.5), fontsize=ticksize, framealpha=0.9)
    ...
    dataDict = {
        ...
        'observed data':{
            ...
            'xo_grid': xo_grid.tolist(),
            'yo_grid': yo_grid.tolist(),
            'uo': uo.tolist()
        },
        'testing data':{
            "N_test": N_test,
    ...
    N_iter = 120000
    # NOTE: gradient with respect to lambda small (small update every iteration)
    # NOTE: a bigger gap compared to forward, most likely from observation data
    loss_value_step = 100
    pred_step = 2500
    contour_step = 10000 # if not pred_step's multiple, graph every least common multiple (pred_step, contour_step)
    for i in range(N_iter):
        lr = 10**-3 * 2**(-i/10000) if i <= 60000 else 10**-3 * 2**(-60000/10000) #  0.00002210(55000)/0.00001563 # learning rate decay
        ...
        if (i+1) % loss_value_step == 0:  # start with i=9 and end with i=8999 (last iter)
            loss_values.append(float(loss_value)) # loss_value: numpy.float32
            lambda_1s.append(float(lambda_1[0])) # lambda_1: numpy.ndarray [numpy.float32]
            print('Iter: %d, Loss: %.3e, Lambda_1: %.5f, Time: %.2f, Learning Rate: %.8f' % (i+1, loss_value, lambda_1, time.time() - start_time, lr))
   ...
   ###########################
    ...
    plt.close(fig)
    with open(f'{dirpath}/inverse_2d_loss.json', 'w') as f:
    ...
    fig.subplots_adjust(left=0.13, right=0.98, bottom=0.07, top=0.95)
    plt.savefig(f'{dirpath}/inverse_2d_mse.pdf')
    plt.close(fig)
    with open(f'{dirpath}/inverse_2d_mse.json', 'w') as f:
        json.dump({"x_coords": x_coords.tolist(), "u_mses": u_mses.tolist()}, f)
    
    # Plot 5. lambda_1 vs iteration
    fig = plt.figure(figsize=(4.3, 6))
    plt.ticklabel_format(axis='x', style="sci", scilimits=(3,3))
    ...
    plt.plot([init_tuple[0], last_tuple[0]], [init_tuple[1], last_tuple[1]], '.', c='#3B75AF')
    fig.subplots_adjust(left=0.13, right=0.98, bottom=0.07, top=0.95)
    plt.savefig(f'{dirpath}/inverse_2d_lambda.pdf')
    plt.close(fig)
    with open(f'{dirpath}/inverse_2d_lambda.json', 'w') as f:
        json.dump({"x_coords": x_coords.tolist(), "lambda_1s": lambda_1s}, f)
    
    # Plot 6. u vs (x, y) surface
    fig = plt.figure()
    ...
    infoDict = {
        'problem':{
            ...
            'boundary condition': 'u(0, y) = -2 * y^2, u(1, y) =  1 - 2 * y^2 + 3y, u(x, 0) = x^2, u(x, 1) = x^2 - 2 + 3x',
            'analytical solution': 'u(x, y) = x^2 - 2 * y^2 + 3xy'
        },
        'model':{
            'layers': str(layers),
